...
static void LoadUserModesFromCfg()
{
    ...
    {
        count = VDISPLAY_MAX_USER_MODES;
    }

    for (int i = 0; i < count; i++)
    {
        int w = 0, h = 0, hz = 0;
        if (fscanf_s(file, "%d %d %d", &w, &h, &hz) != 3)
        {
            break;
        }
        ...
        g_UserModes[g_UserModeCount].Width = (DWORD)w;
        g_UserModes[g_UserModeCount].Height = (DWORD)h;
        g_UserModes[g_UserModeCount].VSync = (DWORD)hz;
        g_UserModeCount++;
    }

    fclose(file);

    if (g_UserModeCount == 0)
    {
        return;
    }

    if (preferred < 0 || (DWORD)preferred >= g_UserModeCount)
    {
        preferred = 0;
    }

    g_UserPreferredIdx = (DWORD)preferred;
}

static bool HasUserModes()
{
...
EVT_WDF_DRIVER_DEVICE_ADD VDisplayDeviceAdd;
EVT_WDF_DEVICE_D0_ENTRY VDisplayDeviceD0Entry;

EVT_IDD_CX_ADAPTER_INIT_FINISHED VDisplayAdapterInitFinished;
EVT_IDD_CX_ADAPTER_COMMIT_MODES VDisplayAdapterCommitModes;

EVT_IDD_CX_PARSE_MONITOR_DESCRIPTION VDisplayParseMonitorDescription;
EVT_IDD_CX_MONITOR_GET_DEFAULT_DESCRIPTION_MODES VDisplayMonitorGetDefaultModes;
EVT_IDD_CX_MONITOR_QUERY_TARGET_MODES VDisplayMonitorQueryModes;

EVT_IDD_CX_MONITOR_ASSIGN_SWAPCHAIN VDisplayMonitorAssignSwapChain;
EVT_IDD_CX_MONITOR_UNASSIGN_SWAPCHAIN VDisplayMonitorUnassignSwapChain;
...
_Use_decl_annotations_
NTSTATUS VDisplayDeviceAdd(WDFDRIVER Driver, PWDFDEVICE_INIT pDeviceInit)
{
    ...
    PnpPowerCallbacks.EvtDeviceD0Entry = VDisplayDeviceD0Entry;
    ...
    // IddConfig.EvtIddCxDeviceIoControl = VDisplayIoDeviceControl;

    IddConfig.EvtIddCxAdapterInitFinished = VDisplayAdapterInitFinished;

    IddConfig.EvtIddCxParseMonitorDescription = VDisplayParseMonitorDescription;
    IddConfig.EvtIddCxMonitorGetDefaultDescriptionModes = VDisplayMonitorGetDefaultModes;
    IddConfig.EvtIddCxMonitorQueryTargetModes = VDisplayMonitorQueryModes;
    IddConfig.EvtIddCxAdapterCommitModes = VDisplayAdapterCommitModes;
    IddConfig.EvtIddCxMonitorAssignSwapChain = VDisplayMonitorAssignSwapChain;
    IddConfig.EvtIddCxMonitorUnassignSwapChain = VDisplayMonitorUnassignSwapChain;
...
_Use_decl_annotations_
NTSTATUS VDisplayDeviceD0Entry(WDFDEVICE Device, WDF_POWER_DEVICE_STATE PreviousState)
{
...
SwapChainProcessor::SwapChainProcessor(IDDCX_SWAPCHAIN hSwapChain, shared_ptr<Direct3DDevice> Device, HANDLE NewFrameEvent, UINT MonitorIndex)
    : m_hSwapChain(hSwapChain), m_Device(Device), m_hAvailableBufferEvent(NewFrameEvent), m_MonitorIndex(MonitorIndex),
      m_hLayoutMap(nullptr), m_hFramesMap(nullptr), m_pLayout(nullptr), m_pFrames(nullptr)
{
...
SwapChainProcessor::~SwapChainProcessor()
{
    ...
    if (m_pLayout)
    {
        UnmapViewOfFile(m_pLayout);
        m_pLayout = nullptr;
    }

    if (m_pFrames)
    {
        UnmapViewOfFile(m_pFrames);
        m_pFrames = nullptr;
    }

    if (m_hLayoutMap)
    {
        CloseHandle(m_hLayoutMap);
        m_hLayoutMap = nullptr;
    }

    if (m_hFramesMap)
    {
        CloseHandle(m_hFramesMap);
        m_hFramesMap = nullptr;
    }
}
...
bool SwapChainProcessor::EnsureSharedMemory()
{
    if (m_pLayout && m_pFrames)
    {
        return true;
    }
    ...
    if (!m_hLayoutMap)
    {
        m_hLayoutMap = OpenFileMappingW(FILE_MAP_ALL_ACCESS, FALSE, VDISPLAY_LAYOUT_MAP_NAME);
        if (m_hLayoutMap)
        {
            m_pLayout = reinterpret_cast<VDisplaySharedLayout*>(MapViewOfFile(m_hLayoutMap, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(VDisplaySharedLayout)));
        }
    }

    if (!m_hFramesMap)
    {
        m_hFramesMap = OpenFileMappingW(FILE_MAP_ALL_ACCESS, FALSE, VDISPLAY_FRAMES_MAP_NAME);
        if (m_hFramesMap)
        {
            m_pFrames = reinterpret_cast<VDisplaySharedFrames*>(MapViewOfFile(m_hFramesMap, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(VDisplaySharedFrames)));
        }
    }

    return m_pLayout != nullptr && m_pFrames != nullptr;
}

void SwapChainProcessor::RenderSharedFrame(ID3D11Texture2D* pTarget)
{
    if (!EnsureSharedMemory() || !m_pLayout->CaptureActive || m_MonitorIndex >= m_pLayout->MonitorCount)
    {
        return;
    }

    auto& vm = m_pLayout->Vm[m_MonitorIndex];
    if (!vm.FrameReady)
    {
        return;
    }
    ...
    const UINT copyWidth = min(vm.DstW, targetDesc.Width);
    const UINT copyHeight = min(vm.DstH, targetDesc.Height);
    if (copyWidth == 0 || copyHeight == 0)
    {
        return;
    }

    const UINT srcRowPitch = vm.DstW * 4;
    const UINT slicePitch = srcRowPitch * vm.DstH;
    ...
    m_Device->DeviceContext->UpdateSubresource(
        pTarget,
        0,
        &box,
        m_pFrames->Pixels[m_MonitorIndex],
        srcRowPitch,
        slicePitch);

    InterlockedExchange(&vm.FrameReady, 0);
}
...
void SwapChainProcessor::RunCore()
{
    ...
    for (;;)
    {
        ...
        else if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(AcquiredBuffer.As(&targetTexture)))
            {
                RenderSharedFrame(targetTexture.Get());
            }
...
void IndirectDeviceContext::InitAdapter()
{
    ...
    AdapterCaps.EndPointDiagnostics.pEndPointFriendlyName = L"VDisplay Virtual Monitor";
    AdapterCaps.EndPointDiagnostics.pEndPointManufacturerName = L"VDisplay";
    AdapterCaps.EndPointDiagnostics.pEndPointModelName = L"Virtual Split Monitor";
...
IndirectMonitorContext::IndirectMonitorContext(_In_ IDDCX_MONITOR Monitor, UINT ConnectorIndex) :
    m_Monitor(Monitor), m_ConnectorIndex(ConnectorIndex)
{
}
...
void IndirectMonitorContext::AssignSwapChain(IDDCX_SWAPCHAIN SwapChain, LUID RenderAdapter, HANDLE NewFrameEvent)
{
    ...
    else
    {
        ...
        m_ProcessingThread.reset(new SwapChainProcessor(SwapChain, Device, NewFrameEvent, m_ConnectorIndex));
    }
}
...
_Use_decl_annotations_
NTSTATUS VDisplayAdapterInitFinished(IDDCX_ADAPTER AdapterObject, const IDARG_IN_ADAPTER_INIT_FINISHED* pInArgs)
{
...
_Use_decl_annotations_
NTSTATUS VDisplayAdapterCommitModes(IDDCX_ADAPTER AdapterObject, const IDARG_IN_COMMITMODES* pInArgs)
{
...
_Use_decl_annotations_
NTSTATUS VDisplayParseMonitorDescription(const IDARG_IN_PARSEMONITORDESCRIPTION* pInArgs, IDARG_OUT_PARSEMONITORDESCRIPTION* pOutArgs)
{
    // ==============================
    ...
    pOutArgs->MonitorModeBufferOutputCount = HasUserModes()
        ? g_UserModeCount
        : (UINT)IndirectSampleMonitor::szModeList;

    const UINT modeCount = pOutArgs->MonitorModeBufferOutputCount;
    ...
    else
    {
        if (HasUserModes())
        {
            for (DWORD ModeIndex = 0; ModeIndex < g_UserModeCount; ModeIndex++)
            {
                pInArgs->pMonitorModes[ModeIndex] = CreateIddCxMonitorMode(
                    g_UserModes[ModeIndex].Width,
                    g_UserModes[ModeIndex].Height,
                    g_UserModes[ModeIndex].VSync,
                    IDDCX_MONITOR_MODE_ORIGIN_MONITORDESCRIPTOR
                );
            }

            pOutArgs->PreferredMonitorModeIdx = g_UserPreferredIdx;
            return STATUS_SUCCESS;
        }
...
_Use_decl_annotations_
NTSTATUS VDisplayMonitorGetDefaultModes(IDDCX_MONITOR MonitorObject, const IDARG_IN_GETDEFAULTDESCRIPTIONMODES* pInArgs, IDARG_OUT_GETDEFAULTDESCRIPTIONMODES* pOutArgs)
{
    ...
    if (HasUserModes())
    {
        ...
        {
            pOutArgs->DefaultMonitorModeBufferOutputCount = g_UserModeCount;
        }
        else
        {
            for (DWORD ModeIndex = 0; ModeIndex < g_UserModeCount; ModeIndex++)
...
_Use_decl_annotations_
NTSTATUS VDisplayMonitorQueryModes(IDDCX_MONITOR MonitorObject, const IDARG_IN_QUERYTARGETMODES* pInArgs, IDARG_OUT_QUERYTARGETMODES* pOutArgs)
{
    UNREFERENCED_PARAMETER(MonitorObject);
    ...
    if (HasUserModes())
    {
        for (DWORD i = 0; i < g_UserModeCount; i++)
        {
            TargetModes.push_back(CreateIddCxTargetMode(
                g_UserModes[i].Width,
                g_UserModes[i].Height,
                g_UserModes[i].VSync));
        }
    }
    else
    {
        ...
        TargetModes.push_back(CreateIddCxTargetMode(1280, 1080, 60));
        ...
        TargetModes.push_back(CreateIddCxTargetMode( 720,  720, 60));
    }
...
_Use_decl_annotations_
NTSTATUS VDisplayMonitorAssignSwapChain(IDDCX_MONITOR MonitorObject, const IDARG_IN_SETSWAPCHAIN* pInArgs)
{
...
_Use_decl_annotations_
NTSTATUS VDisplayMonitorUnassignSwapChain(IDDCX_MONITOR MonitorObject)
{
    auto* pMonitorContextWrapper = WdfObjectGet_IndirectMonitorContextWrapper(MonitorObject);
